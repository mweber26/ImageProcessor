...
#pragma warning restore 3001 // void* is not a CLS-Compliant argument type
        ...
        public unsafe void CopyTo(T[] destination, int startIndex)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(string.Format("Index was out of bounds:", startIndex));
            }
            if ((destination.Length - startIndex) < Count)
            {
                throw new ArgumentException(string.Format("Number of elements in source vector is greater than the destination array", startIndex));
            }
        ...
        public unsafe T this[int index]
        {
            get
            {
                ...
                {
                    throw new IndexOutOfRangeException(string.Format("Index was out of bounds:", index));
                }
                if (typeof(T) == typeof(Byte))
                {
                    fixed (Byte* basePtr = &this.register.byte_0)
